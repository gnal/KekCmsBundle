...
abstract class Admin
{
    ...
    public function buildParentBreadcrumb(&$breadcrumb, $parent, $object)
    {
        ...
        $breadcrumb->add(
            '<span class="fa-list fa fa-lg"></span> '.ucfirst($parent->getLabel(2)),
            $parent->genUrl('index', [
        ...
        $breadcrumb->add(
            '<span class="fa-file fa fa-lg"></span> '.$object,
            $parent->genUrl('edit', [
                'id' => $object->getId(),
                'parentId' => $parent->hasParent() ? $object->$getter()->getId() : null,
            ], false)
    ...
    public function buildBaseBreadcrumb($action)
    {
        ...
        $breadcrumb
            ->add('<span class="fa-list fa fa-lg"></span> '.ucfirst($this->getLabel(2)), $action === 'list' ? null : $this->genUrl('index'))
        ;
    ...
    public function buildIndexBreadcrumb($breadcrumb)
    {
        if ($this->hasParent()) {
            $breadcrumb
                // ->add('<span class="icon-share-alt fa fa-lg"></span> '.ucfirst($this->container->get('translator')->trans('back')), $this->getParent()->genUrl('index'), [
                //     'attr' => [
                //         'class' => 'pull-right',
                //     ],
                // ])
    ...
    {
        $breadcrumb
            ->add('<span class="fa-plus fa fa-lg"></span> '.ucfirst($this->container->get('translator')->trans('add')))
            // ->add('<span class="icon-share-alt fa fa-lg"></span> '.ucfirst($this->container->get('translator')->trans('back')), $this->genUrl('index'), [
            //     'attr' => [
            //         'class' => 'pull-right',
            //     ],
            // ])
    ...
    {
        $breadcrumb
            ->add('<span class="fa-file fa fa-lg"></span> '.$this->getObject(), $this->genUrl('edit', ['id' => $this->getObject()->getId()]))
            ->add('<span class="fa-pencil fa fa-lg"></span> '.ucfirst($this->container->get('translator')->trans('edit')))
            // ->add('<span class="icon-share-alt fa fa-lg"></span> '.ucfirst($this->container->get('translator')->trans('back')), $this->genUrl('index'), [
            //     'attr' => [
            //         'class' => 'pull-right',
            //     ],
    ...
    {
        $breadcrumb
            ->add('<span class="icon-file-alt fa fa-lg"></span> '.$this->getObject())
        ;
    }
